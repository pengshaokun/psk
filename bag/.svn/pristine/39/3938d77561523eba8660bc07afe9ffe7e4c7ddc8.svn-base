...
public class WebMessageController {
    ...
    public Object addMessage(@RequestBody MessageParam messageParam) {
        try {
           ...
           // if (loginInfo == null) return ReturnMapByBack.result(0, "用户未登录,请先登录!");
            ...
            if (flag > 0) return ReturnMapByBack.result(1, "消息添加成功!");
            else return ReturnMapByBack.result(0, "消息添加失败!");
    ...
    public Object deleteMessage(@PathVariable int id) {
        try {
            ...
            if (flag > 0) return ReturnMapByBack.result(1, "消息删除成功!");
            else return ReturnMapByBack.result(0, "消息删除失败!");
    ...
    public Object updateMessage(@RequestBody MessageParam messageParam) {
        try {
            ...
            if (flag > 0) return ReturnMapByBack.result(1, "消息修改成功!");
            else return ReturnMapByBack.result(0, "消息修改失败!");
        } catch (Exception e) {
            e.printStackTrace();
            return ReturnMapByBack.result(0, "服务异常,请联系管理员!");
        }
    ...
    public Object queryById(@PathVariable int id) {
        try {
            ...
            if (messageEntry != null) {
                return ReturnMapByBack.result(1, "获取消息详情成功!", messageEntry);
            } else {
                return ReturnMapByBack.result(0, "获取消息详情失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return ReturnMapByBack.result(0, "服务异常,请联系管理员!");
    ...
    @ResponseBody
    public Object query(Integer page, Integer rows, MessageParam messageParam) {
        try {
            LoginInfo loginInfo = loginContext.getInfoWeb();
            // if (loginInfo == null) return ReturnMapByBack.result(0, "用户未登录,请先登录!");
            List<MessageEntry> list =  messageService.query(page, rows, messageParam);
            // 查询列表数量
            int count = messageService.queryAllMessageCount(messageParam);
            return ReturnMapByBack.result(1, "获取消息列表成功!", list, count);
        } catch (Exception e) {
            e.printStackTrace();
            return ReturnMapByBack.result(0, "服务异常,请联系管理员!");
        }
